...
class ResNet(nn.Layer):
    ...
    def __init__(self, layers, zero_init_residual=False,
        ...
        self.relu = nn.ReLU()
        self.maxpool1 = nn.MaxPool2D(kernel_size=3, stride=2, padding=1, return_mask=True)
        self.maxpool2 = nn.MaxPool2D(kernel_size=3, stride=2, padding=1, return_mask=True)
        self.maxpool3 = nn.MaxPool2D(kernel_size=3, stride=2, padding=1, return_mask=True)
        self.maxpool4 = nn.MaxPool2D(kernel_size=3, stride=2, padding=1, return_mask=True)
        self.maxpool5 = nn.MaxPool2D(kernel_size=3, stride=2, padding=1, return_mask=True)
        #pdb.set_trace()
        self.layer1 = self._make_layer(BasicBlock, 64, layers[0])
        self.layer2 = self._make_layer(BasicBlock, 128, layers[1], stride=1,
                                       dilate=replace_stride_with_dilation[0])
        self.layer3 = self._make_layer(BasicBlock, 256, layers[2], stride=1,
        ...
        if zero_init_residual:
            for m in self.sublayers():
                if isinstance(m, Bottleneck):
                    m.bn3.weight = nn.initializer.Constant(value = 0)
                    #nn.init.constant_(m.bn3.weight, 0)
                elif isinstance(m, BasicBlock):
                    m.bn2.weight = nn.initializer.Constant(value = 0)
...
def resnet34_mp(pretrained=None,**kwargs):
    r"""ResNet-34 model from
    `"Deep Residual Learning for Image Recognition" <https://arxiv.org/pdf/1512.03385.pdf>`
    """
    model = ResNet(BasicBlock, [3, 4, 6, 3], **kwargs)
    paddle_state_dict = paddle.load(pretrained)
